...
public class HTMXController : Controller
{
    ...
    public HTMXController(ILogger<HTMXController> logger, SqliteContext context)
    {
        _logger = logger;
        _context = context;
    ...
    public IActionResult AccountsList([FromQuery] int? size) {
        size ??= 100;
        var accounts = _context.Accounts.AsNoTracking().Take(size.Value).ToList();
    ...
    public async Task<IActionResult> AccountsListFilter(string search, [FromQuery] int? size) {
        ...
        if (search != null && !search.Equals("")) {
            ...
            accounts = _context.Accounts.AsNoTracking().Take(size.Value).ToList();
    ...
    public IActionResult Scroll([FromQuery] int? size, [FromQuery] int? offset = 0, int? id = 1, [FromQuery] bool? split = false) {
        size ??= 100;
        DemoObject d = _context.TableContainer.AsNoTracking().First(m => m.Id == id);
        var table = 
    ...
    public IActionResult Demo(int? sortIdx, [FromQuery] int? size, [FromQuery] int id = 1) {
        ...
        List<DemoTable> table = 
            (from row in _context.Entries.AsNoTracking() where row.DemoObjectId == d.Id select row).Take(size.Value).ToList();
        d.Table = table;
